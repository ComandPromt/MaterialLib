...
public class ButtonTask extends JButton {
	...
	private Dimension computeDimension() {
		...
		clickable = new Rectangle(insets.left, insets.top /* + 4 */, width /* + insets.left + insets.right */,
	...
	protected void paintComponent(Graphics g) {
		...
		if (isOpaque()) {
			...
			if (redondeado)
				g2.fillRoundRect(x, y, getWidth() - (5 + (int) anchoDeBorde), getHeight() - (5 + (int) anchoDeBorde),
						25, 25);
			else
				g2.fillRect(x + (int) anchoDeBorde, y, getWidth() - (5 + (int) anchoDeBorde),
						getHeight() - (5 + (int) anchoDeBorde));
	...
	private float paintText(Graphics2D g2) {
		...
		TextLayout layout = new TextLayout(getText(), categoryFont, context);
