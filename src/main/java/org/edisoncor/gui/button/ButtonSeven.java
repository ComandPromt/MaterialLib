...
public class ButtonSeven extends JButton {
	...
	private BufferedImage mainButton, mainButtonPressed, normalButton, normalButtonPressed, buttonHighlight,
			buttonHighlight2;
...
//        if (ghostValue > 0.0f) {
		...
		if (x != null) {
			g2.drawImage(buttonHighlight2, x - (buttonHighlight2.getWidth() / 2), 0, buttonHighlight2.getWidth(),
					buttonHighlight2.getHeight(), null);
		}
...
//        }
		...
		int xx = (int) (getWidth() - insets.left - insets.right - bounds.getWidth()) / 2;
		// x -= 2;
		...
		layout.draw(g2, xx + (int) Math.ceil(shadowOffsetX), y + (int) Math.ceil(shadowOffsetY));
		if (isEnabled())
