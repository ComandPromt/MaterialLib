...
public class MaterialShadowRectangle {
	...
	private static KeyFrames<Float> opacity1 = (new KeyFrames.Builder<Float>(Float.valueOf(0.0F)))
	...
	private static KeyFrames<Float> opacity2 = (new KeyFrames.Builder<Float>(Float.valueOf(0.0F)))
	...
	private static KeyFrames<Float> radius1 = (new KeyFrames.Builder<Float>(Float.valueOf(0.0F)))
	...
	private static KeyFrames<Float> radius2 = (new KeyFrames.Builder<Float>(Float.valueOf(0.0F)))
	...
	private static KeyFrames<Float> offset1 = (new KeyFrames.Builder<Float>(Float.valueOf(0.0F)))
	...
	private static KeyFrames<Float> offset2 = (new KeyFrames.Builder<Float>(Float.valueOf(0.0F)))
	...
	public static BufferedImage renderShadow(int width, int height, double level) {
		...
		if (width > 0 && height > 0 && level != 0.0D) {
			...
			makeShadow(shadow, opacity1.getInterpolatedValueAt(level / 5.0D).floatValue(),
					radius1.getInterpolatedValueAt(level / 5.0D).floatValue(), 0.0F,
					offset1.getInterpolatedValueAt(level / 5.0D).floatValue());

			makeShadow(shadow2, opacity2.getInterpolatedValueAt(level / 5.0D).floatValue(),
					radius2.getInterpolatedValueAt(level / 5.0D).floatValue(), 0.0F,
					offset2.getInterpolatedValueAt(level / 5.0D).floatValue());
	...
	public static BufferedImage renderCircularShadow(int radius, double level) {
		...
		if (level != 0.0D) {
			...
			makeCircularShadow(shadow, opacity1.getInterpolatedValueAt(level / 5.0D).floatValue(),
					radius1.getInterpolatedValueAt(level / 5.0D).floatValue(), 0.0F,
					offset1.getInterpolatedValueAt(level / 5.0D).floatValue());

			makeCircularShadow(shadow2, opacity2.getInterpolatedValueAt(level / 5.0D).floatValue(),
					radius2.getInterpolatedValueAt(level / 5.0D).floatValue(), 0.0F,
					offset2.getInterpolatedValueAt(level / 5.0D).floatValue());
	...
	private static void makeShadow(BufferedImage shadow, float opacity, float radius, float leftOffset,
		...
		g2.fill(new RoundRectangle2D.Float(5.0F + leftOffset, 5.0F + topOffset, (shadow.getWidth() - 5 - 5),
				(shadow.getHeight() - 5 - 5), 30.0F, 30.0F));
