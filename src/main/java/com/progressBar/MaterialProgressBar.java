...
public class MaterialProgressBar extends JProgressBar {
	...
	public MaterialProgressBar() {
		...
		colorDeBorde = new Color(0, 51, 51);

		colorDeSombra = new Color(0, 0, 0);

		colorDeTexto = new Color(255, 255, 255);
	...
	protected void paintComponent(Graphics g) {
		...
		LinearGradientPaint gr = new LinearGradientPaint(0, 0, 0, getHeight(), new float[] { 0f, 0.3f, 0.5f, 1f },
				new Color[] { getForeground().brighter(), getForeground(), getForeground().darker().darker(),
						getForeground().darker() });

		g2.setPaint(gr);

		g2.fillRoundRect(0, 0, getWidth() * getValue() / 100, getHeight(), angulo, angulo);
