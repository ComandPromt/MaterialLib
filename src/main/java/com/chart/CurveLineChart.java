...
public class CurveLineChart extends JComponent {
	...
	private void createChart() {
		...
		blankPlotChart.setBlankPlotChartRender2(new BlankPlotChartRender2() {
			...
			public void renderGraphics(BlankPlotChart2 chart, Graphics2D g2, Rectangle2D rectangle) {
				...
				if (!model.isEmpty() && animate > 0 && index >= 0 && index <= legends.size() - 1) {
					...
					draw(g2, rectangle, index,
							chart.getNiceScale().getTickSpacing() * chart.getNiceScale().getMaxTicks());
			...
			public void mouseMove(Rectangle2D rectangle, MouseEvent mouse) {
				...
				if (!model.isEmpty()) {
					...
					for (int i = 0; i < per; i++) {
						...
						if (mouse.getX() >= x && mouse.getX() <= x + per && mouse.getY() >= rectangle.getY()
	...
	public void addLegend(String name, Color color1, Color color2) {
		...
		LegendItem2 legend = new LegendItem2(data, legends.size() - 1);
	...
	private void clearLegendSelected(LegendItem2 item) {

		item.setSelected(true);

		for (Component com : panelLegend.getComponents()) {

			if (com instanceof LegendItem2) {

				LegendItem2 l = (LegendItem2) com;
