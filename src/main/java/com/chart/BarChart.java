...
public class BarChart extends JComponent {
	...
	private BlankPlotChart1 blankPlotChart;
	...
	private void createBlankChart() {
		...
		blankPlotChart = new BlankPlotChart1();
	...
	private void createChart() {
		...
		blankPlotChart.setBlankPlotChartRender(new BlankPlotChartRender1() {
			...
			public void renderSeries(BlankPlotChart1 chart, Graphics2D g2, SeriesSize size, int index) {
				...
				for (int i = 0; i < legends.size(); i++) {
					...
					RoundRectangle2D r2d = new RoundRectangle2D.Double(size.getX() + x,

							size.getY() + size.getHeight() - seriesValuesAnimation, seriesSize, seriesValuesAnimation,
				...
				if (labelText != null) {
					...
					g2.fill(new RoundRectangle2D.Double(labelLocation.getX() - s.getWidth() / 2 - 3,

							labelLocation.getY() - s.getHeight() - space * 2 - spaceTop, s.getWidth() + space * 2,

							s.getHeight() + space * 2, 10, 10));
			...
			@Override
			public boolean mouseMoving(BlankPlotChart1 chart, MouseEvent evt, Graphics2D g2, SeriesSize size,
					int index) {
				...
				for (int i = 0; i < legends.size(); i++) {
					...
					if (r2d.contains(evt.getPoint())) {
						...
						if (overIndex != seriesIndex) {
							...
							Point newPoint = new Point((int) (size.getX() + x + s),
